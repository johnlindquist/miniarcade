...
for(const seed of[0x6510,0x6511]){
  ...
  const evidence={
    ...
    decisions:{puzzle:p.stats.puzzleSteps,threat:p.stats.engagements,response:p.stats.pulses+p.stats.dodges,combat:p.stats.stuns,payoff:p.stats.loot+p.stats.escapes},
  ...
  if(p.stats.engagements<350||p.stats.cutoffs<100||p.stats.pulses<140||p.stats.dodges<550)fail(seed.toString(16)+': enemies/player did not visibly adapt '+JSON.stringify(p.stats));
...
for(const seed of[0x6520,0x6521,0x6522,0x6523,0x6524,0x6525]){
  ...
  if(pa.stats.pulses<70||pa.stats.dodges<250||pa.stats.cutoffs<45)fail(seed.toString(16)+': tactical policy became inactive');
