...
if(games.length!==18||ids.size!==games.length)fail('manifest must contain eighteen unique game ids');
...
if(!index.includes('eighteen tiny games'))fail('gallery count copy is stale');
...
for(const tone of ['sy','af','tl','cc','mb','gs'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('eighteen self-playing'))fail('runtime README count is stale');
if(!index.includes('scroll-snap-type:x proximity'))fail('gallery rail does not provide horizontal carousel snapping');
if(!index.includes("card.addEventListener('pointermove'"))fail('gallery cards do not expand on hover');
if(index.includes('pointerenter')||index.includes('hoverLockUntil'))fail('hover activation must be movement-gated pointermove, not pointerenter/time-lock hacks');
...
if(!index.includes('(hover:none),(pointer:coarse)'))fail('gallery does not provide full-width touch cards with a visible PLAY affordance');
