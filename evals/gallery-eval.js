...
if(games.length!==23||ids.size!==games.length)fail('manifest must contain twenty-three unique game ids');
...
if(!index.includes('Twenty-three tiny, self-playing retro games')||!index.includes('twenty-three tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 23</output>')||index.includes('>01 / 22</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp','bb','rc'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Twenty-three self-playing'))fail('runtime README count is stale');
