...
if(games.length!==32||ids.size!==games.length)fail('manifest must contain thirty-two unique game ids');
...
if(!index.includes('Thirty-two tiny, self-playing retro games')||!index.includes('thirty-two tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 32</output>')||index.includes('>01 / 31</output>'))
...
if(!readme.includes('Thirty-two self-playing'))fail('runtime README count is stale');
