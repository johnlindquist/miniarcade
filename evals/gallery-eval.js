...
if(games.length!==21||ids.size!==games.length)fail('manifest must contain twenty-one unique game ids');
...
if(!index.includes('Twenty-one tiny, self-playing retro games')||!index.includes('twenty-one tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 21</output>')||index.includes('>01 / 20</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Twenty-one self-playing'))fail('runtime README count is stale');
