...
if(!index.includes('Eighteen tiny, self-playing retro games')||!index.includes('eighteen tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 18</output>')||index.includes('>01 / 17</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
