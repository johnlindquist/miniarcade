...
if(games.length!==20||ids.size!==games.length)fail('manifest must contain twenty unique game ids');
...
if(!index.includes('Twenty tiny, self-playing retro games')||!index.includes('twenty tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 20</output>')||index.includes('>01 / 19</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Twenty self-playing'))fail('runtime README count is stale');
