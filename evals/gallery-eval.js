...
if(games.length!==29||ids.size!==games.length)fail('manifest must contain twenty-nine unique game ids');
...
if(!index.includes('Twenty-nine tiny, self-playing retro games')||!index.includes('twenty-nine tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 29</output>')||index.includes('>01 / 28</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp','bb','rc','rr','cc','hh','kc','mv','de'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Twenty-nine self-playing'))fail('runtime README count is stale');
