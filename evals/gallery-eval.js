...
if(games.length!==19||ids.size!==games.length)fail('manifest must contain nineteen unique game ids');
...
if(!index.includes('Nineteen tiny, self-playing retro games')||!index.includes('nineteen tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 19</output>')||index.includes('>01 / 18</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Nineteen self-playing'))fail('runtime README count is stale');
