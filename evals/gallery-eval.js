...
if(games.length!==33||ids.size!==games.length)fail('manifest must contain thirty-three unique game ids');
...
if(!index.includes('Thirty-three tiny, self-playing retro games')||!index.includes('thirty-three tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 33</output>')||index.includes('>01 / 32</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp','bb','rc','rr','cc','hh','kc','mv','de','ar','rf'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Thirty-three self-playing'))fail('runtime README count is stale');
