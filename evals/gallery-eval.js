...
if(games.length!==36||ids.size!==games.length)fail('manifest must contain thirty-six unique game ids');
...
if(!index.includes('Thirty-six tiny, self-playing retro games')||!index.includes('thirty-six tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 36</output>')||index.includes('>01 / 33</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp','bb','rc','rr','cc','hh','kc','mv','de','ar','sr','pz','df','rf'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Thirty-six self-playing'))fail('runtime README count is stale');
