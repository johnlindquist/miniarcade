...
if(games.length!==24||ids.size!==games.length)fail('manifest must contain twenty-four unique game ids');
...
if(!index.includes('Twenty-four tiny, self-playing retro games')||!index.includes('twenty-four tiny games'))
  fail('gallery count copy is stale');
if(!index.includes('>01 / 24</output>')||index.includes('>01 / 23</output>'))
  fail('gallery position count is stale');
if(!index.includes('.ss{--accent:'))fail('Scrap Shift gallery tone is missing');
for(const tone of ['mb','gs','wr','gg','sk','sv','ng','pc','fc','tp','bb','rc','rr'])if(!index.includes(`.${tone}{--accent:`))fail(`${tone} gallery tone is missing`);
...
if(!readme.includes('Twenty-four self-playing'))fail('runtime README count is stale');
