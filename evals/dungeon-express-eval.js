...
for(const seed of[0xdf010,0xdf011,0xe7015]){
  ...
  const layout=game.sandbox.__dungeonExpressLayoutFixture(),degree=layout.rooms.map(room=>layout.edges.filter(e=>e.a===room.id||e.b===room.id).length);
  assertEntertainment(seed.toString(16),{noVisiblePath,topology:{rooms:layout.rooms.length,branches:degree.filter(value=>value>=3).length,maxStraight:longestStraight(layout.grid)},puzzle:{transitions:p.stats.puzzleTransitions,completions:p.stats.puzzleCompletions},agency:{enemyActions:p.stats.enemyActions,playerResponses:p.stats.playerResponses},decisions:{puzzle:{count:p.stats.puzzleTransitions,source:'stats.puzzleTransitions'},threat:{count:p.stats.enemyTells+p.stats.enemyActions,source:'stats.enemyTells+stats.enemyActions'},response:{count:p.stats.playerResponses,source:'stats.playerResponses'},combat:{count:p.stats.combatBeats,source:'stats.combatBeats'},payoff:{count:p.stats.payoffs,source:'stats.payoffs'}},maxDeadAir:p.stats.maxDecisionDeadAir},{minRooms:6,minBranches:2,maxStraight:9,minPuzzleTransitions:105,minPuzzleCompletions:42,minEnemyActions:165,minPlayerResponses:155,requiredDecisionKinds:['puzzle','threat','response','combat','payoff'],minPerDecisionKind:100,maxDeadAir:280,deadAirUnit:'tactical frames'},fail);
