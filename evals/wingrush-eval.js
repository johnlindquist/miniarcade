...
const targetMetrics=p=>({
  ...
  guidedRatio:ratio(p.stats.guidedLandings,p.stats.landings),roughRatio:ratio(p.stats.roughLandings,p.stats.landings),
...
// .875..1 completed-guidance ratio, 0..50% per-seed rough ratio (25.3% aggregate),
// and no micro-flights. Per-seed rough variance stays paired to a strict aggregate gate below.
const NATURAL={targets:[7,12],hits:[6,11],hitRatio:[.70,.93],first:[180,520],
  ...
  landings:[7,11],launches:[7,11],x:[6500,11500],guidedAttempts:[7,12],guidedRatio:[.85,1],roughRatio:[0,.55],micro:[0,1]};
...
// The reactive ablation remained active and produced 93 hits; planner aggregate
// was 161 hits/topples, 310 cores, and 10/10 per-seed wins.
...
// Final independent ten-minute seeds 0x7810/11 observed 91..92 targets,
// 72..81 direct hits, ratios .783..890, first hit 240..803f, 1,747..1,802 coins,
// 17,151..19,015 controlled-air frames, 85..87 landings, .976..1 completed
// guidance, 21..24% rough landings, and zero micro-flights.
const SOAK={targets:[86,98],hits:[68,85],hitRatio:[.74,.92],first:[180,900],
  minLead:[140,210],avgLead:[350,410],coins:[1650,1900],air:[16000,20000],
...
console.log('4) sky forts, direct core powers, physical collapse, and pure planner');
{
  const game=bootGame('wingrush',{seed:0x7620,footer:FOOTER}),catalog=game.sandbox.__wrSkyCatalog(),
    look=game.sandbox.__wrPlannerFixture(),control=bootGame('wingrush',{seed:0x7621,footer:FOOTER}),
    planned=bootGame('wingrush',{seed:0x7621,footer:FOOTER});
  planned.sandbox.__wrPlannerFixture();control.sandbox.__wrPlannerFixture();
...
console.log('5) ten paired two-minute seeds: planner beats __NO_LOOKAHEAD on direct target hits');
{
  ...
  for(let i=0;i<10;i++){
    ...
    if(!pb.finite||pb.stats.invisibleResets!==0||pb.stats.targets<15||pb.bird.x<17000||pb.stats.targetHits<8||pb.stats.microHops>2)fail(`seed ${seed.toString(16)} reactive policy left the active/safe ablation contract`);
    if(pa.stats.targetHits>pb.stats.targetHits)hitWins++;
    console.log(`  ${seed.toString(16)} ${pa.persona.padEnd(11)} ${pa.stats.targetHits}/${pa.stats.targets} hits, ${pa.stats.towersToppled} topples vs reactive ${pb.stats.targetHits}/${pb.stats.targets}, ${pb.stats.towersToppled}`);
  }
...
if(ratio(guided,landOn)<.9||roughOn>landOn*.27||roughOn>=roughOff*.30)fail(`landing guidance did not clearly improve truthful touchdowns: ${JSON.stringify({guided,attempts,roughOn,roughOff,landOn,landOff})}`);
