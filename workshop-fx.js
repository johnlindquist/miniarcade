...
(()=>{
  ...
  onReady(()=>{
    ...
    const playBoop=()=>{
      ...
      if(!boopAudio)boopAudio=new AudioContext();
      const play=()=>{
        const t=boopAudio.currentTime;
        const oscillator=boopAudio.createOscillator();
        const gain=boopAudio.createGain();
        oscillator.type='sine';
        oscillator.frequency.setValueAtTime(340,t);
        oscillator.frequency.exponentialRampToValueAtTime(150,t+0.09);
        oscillator.frequency.exponentialRampToValueAtTime(235,t+0.17);
        gain.gain.setValueAtTime(0,t);
        gain.gain.linearRampToValueAtTime(0.15,t+0.008);
        gain.gain.exponentialRampToValueAtTime(0.001,t+0.24);
        oscillator.connect(gain);
        gain.connect(boopAudio.destination);
        oscillator.start(t);
        oscillator.stop(t+0.28);
      };
      if(boopAudio.state==='suspended')boopAudio.resume().then(play).catch(()=>{});
      else play();
    };
